...
Для продуктовой реализации дописать собственный `CarDesign/Low Poly` shader под
актуальные Unity 6 и URP. Старый пакет Minimalist можно использовать как визуальный
референс и временную поддержку legacy-материалов, но не как долгосрочную runtime-
зависимость: его URP-версия устарела и уже несовместима с текущим `MetaPass`.

Собственный shader должен поддерживать крупные читаемые плоскости, направленные
градиенты по локальным или мировым осям, базовый цвет/текстуру, тени и SRP Batcher.
Он должен быть отдельным режимом представления той же геометрии и сочетаться со
скетчем, контуром и wireframe без изменения данных машины.
...
# Colored Pencils
https://assetstore.unity.com/packages/vfx/shaders/fullscreen-camera-effects/colored-pencils-122848#reviews
